...
def load_model_and_tokenizer(trained_model='gpt2', model_type='integral'):
    ...
    if model_type in ['integral', 'syntactic']:
        ...
        model = GPT2LMHeadModelSemantic.from_pretrained(trained_model, 
                                            output_hidden_states=True, 
                                            output_attentions=True,
                                            pad_token_id=pad_token_id)
    return model, tokenizer

def pad_to_max_length(sequence, max_seq_length, space=220, special_token_end=50256, special_token_pad=None):
...
def create_examples(sequence, max_seq_length, space=220, special_token_beg=50256, special_token_end=50256, special_token_pad=None):
    ...
    if space is not None:
        return pad_to_max_length([special_token_beg] + sequence + [space, special_token_end], max_seq_length, space=space, special_token_pad=special_token_pad)
    else:
        return pad_to_max_length([special_token_beg] + sequence + [special_token_end], max_seq_length, space=space, special_token_pad=special_token_pad)
