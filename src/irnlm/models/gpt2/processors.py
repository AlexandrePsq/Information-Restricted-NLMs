...
class ModelProcessor(object):
    ...
    def attention_mask_from_inputs(self, input_ids, context_size):
        ...
        if context_size is None:
            ...
            attention_mask = torch.diag_embed(
                torch.tensor([0 for x in range(input_ids.size(-1))])
            )
            for i in range(
                min(input_ids.size(-1), context_size + 1)
            ):  # Adding 1 so that context==0 is only the current word
                attention_mask = torch.add(
                    attention_mask,
                    torch.diag_embed(
                        torch.tensor([1 for x in range(input_ids.size(-1) - i)]),
                        offset=-i,
                    ),
                )
            attention_mask = (
    ...
    def training_step(self, batch, total_train_loss):
        ...
        attention_mask = (
            None  # torch.ones(batch[0].size()).to(torch.int64).to(self.device)
        )
    ...
    def train(
        ...
        try:
            ...
            special_token_pad_ids = self.tokenizer(self.tokenizer.pad_token)[
                "input_ids"
            ][0]
            space = None
        except ValueError:
        ...
        for epoch_i in range(0, self.nb_epochs):
            ...
            if epoch_i >= parameters["start_epoch"]:
                if epoch_i == 0 and start_at_dataloader == 0:
                    logging.info(
                        "Saving model at the start of epoch {} to {}...".format(
                            epoch_i, os.path.join(output_dir, f"start-epoch-{epoch_i}")
                        )
                    )
                    save_checkpoint(
                        self.model, self.tokenizer, output_dir, f"start-epoch-{epoch_i}"
                    )
                    logging.info("\tDone.")
                ...
                if parameters["do_train"]:
                    ...
                    for split_index, batch_path in enumerate(train_data_paths):
                        if (split_index >= start_at_dataloader) or (
                            start_at_dataloader + 1 == len(train_data_paths)
                        ):
                            start_at_dataloader = 0
                            logging.info(
                                f"[{get_timestamp()}] - Creating training data loader for split {split_index}.."
                            )
                            ...
                            features = [
                                ...
                                for example in tqdm(examples_ids)
                            ]
                            ...
                            for step, batch in enumerate(dataloader):
                                ...
                                total_train_loss = self.training_step(
                                    batch, total_train_loss
                                )
                            ...
                            # Saving
                            logging.info(
                                "Saving model at the end of DataLoader {} to {}...".format(
                                    split_index,
                                    os.path.join(
                                        output_dir,
                                        f"end-epoch-{epoch_i}_split-{split_index}",
                                    ),
                                )
                            )
                            save_checkpoint(
                    ...
                    logging.info(
                        "Saving model at the end of epoch {} to {}...".format(
                            epoch_i, os.path.join(output_dir, f"end-epoch-{epoch_i}")
                        )
                    )
                    save_checkpoint(
                        self.model, self.tokenizer, output_dir, f"end-epoch-{epoch_i}"
                    )
                    logging.info("\tDone.")
                    ...
                    df.to_csv(
                        os.path.join(output_dir, "training_stats.csv"), index=False
                    )

                if parameters["do_validation"]:
                    val_loss, val_time = self.evaluate(
                        data_processor,
                        validation_features_paths,
                        "dev",
                    ...
                    df.to_csv(
                        os.path.join(output_dir, "validation_stats.csv"), index=False
                    )
                ...
                if parameters["do_validation"]:
                    try:
                        ...
                        df = pd.read_csv(
                            os.path.join(output_dir, "validation_stats.csv")
                        )
                        validation_stats = df.to_dict("records")
                    except:
                        if (epoch_i + 1) >= parameters["start_epoch"]:
                            logging.info(
                                f"... but computing the validation loss for previous epoch number {epoch_i}..."
                            )
                            val_loss, val_time = self.evaluate(
                                data_processor,
                                validation_features_paths,
    ...
    def evaluate(self, data_processor, validation_features_paths, set_type, parameters):
        ...
        for split_index, batch_path in enumerate(validation_features_paths):
            ...
            for batch in tqdm(dataloader):
                ...
                label_ids = (
                    input_ids.clone()
                )  # batch[2].to(torch.int64).to(self.device)
                # label_ids[:, 0] = -100
                # label_ids[:, -2] = -100
                # label_ids[:, -1] = -100
