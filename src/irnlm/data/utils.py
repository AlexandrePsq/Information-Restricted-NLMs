...
def get_pronouns(language="english", folder="data"):
    """Returns the list of pronouns for the chosen language."""
    if language == "english":
        result = (
            open(os.path.join(folder, "english_pronouns.txt"), "r").read().split("\n")
        )
    elif language == "french":
        result = (
            open(os.path.join(folder, "french_pronouns.txt"), "r").read().split("\n")
        )
    else:
...
def get_negations(language="english", folder="data"):
    """Returns the list of negations for the chosen language."""
    if language == "english":
        result = (
            open(os.path.join(folder, "english_negations.txt"), "r").read().split("\n")
        )
    elif language == "french":
        result = (
            open(os.path.join(folder, "french_negations.txt"), "r").read().split("\n")
        )
    else:
        raise ValueError(f"Language {language} unknown.")
    return result
...
def get_positions_words(language="english", folder="data"):
    """Returns the list of positions for the chosen language."""
    if language == "english":
        result = (
            open(os.path.join(folder, "english_positions.txt"), "r").read().split("\n")
        )
    elif language == "french":
        result = (
            open(os.path.join(folder, "french_positions.txt"), "r").read().split("\n")
        )
    else:
        raise ValueError(f"Language {language} unknown.")
    return result
...
def get_quantity_words(language="english", folder="data"):
    """Returns the list of quantity for the chosen language."""
    if language == "english":
        result = (
            open(os.path.join(folder, "english_quantity.txt"), "r").read().split("\n")
        )
    elif language == "french":
        result = (
            open(os.path.join(folder, "french_quantity.txt"), "r").read().split("\n")
        )
    else:
        raise ValueError(f"Language {language} unknown.")
    return result
...
def get_function_words(language="english", folder="data"):
    """Returns the list of funtion words for the chosen language."""
    if language == "english":
        result = (
            open(os.path.join(folder, "english_function_words.txt"), "r")
            .read()
            .split("\n")
        )
    elif language == "french":
        result = (
            open(os.path.join(folder, "french_function_words.txt"), "r")
            .read()
            .split("\n")
        )
    else:
...
def extract_pos_info(token, children_max_size=5):
    ...
    children_pos = (
        children_pos[:5]
        if len(children_pos) >= 5
        else (children_pos + ["" for i in range(5 - len(children_pos))])
    )
...
def extract_pipeline_info(token, ner=False):
    ...
    if ner:
        return [
            ...
            token.head.tag_,
            token.shape_,
            str(token.morph) if str(token.morph) != "" else "None",
    ...
    for model_name in model_names:
        ...
        for subject_id in tqdm(possible_subjects_id(language)):
            ...
            path_to_map = os.path.join(
                FMRIDATA_PATH, subject, model_name.format(subject_id)
            )
            R2_maps[subject] = fetch_paths(path_to_map, "*R2.nii.gz")
            R2_maps[subject] = [i for i in R2_maps[subject] if "baseline" not in i]
            Pearson_coeff_maps[subject] = fetch_paths(
                path_to_map, "*Pearson_coeff.nii.gz"
            )
            Pearson_coeff_maps[subject] = [
                i for i in Pearson_coeff_maps[subject] if "baseline" not in i
            ]
            try:
                baseline_R2_maps[subject] = fetch_paths(
                    path_to_map, "*baseline_R2.nii.gz"
                )
                baseline_Pearson_coeff_maps[subject] = fetch_paths(
                    path_to_map, "*baseline_Pearson_coeff.nii.gz"
                )
            except:
            ...
            if verbose > 0:
                print(
                    ...
                    len(R2_maps[subject]),
                    "-",
                    len(Pearson_coeff_maps[subject]),
                    "-",
                    len(baseline_R2_maps[subject]),
                    "-",
                    len(baseline_Pearson_coeff_maps[subject]),
                )
        ...
        try:
            ...
            baseline_Pearson_coeff_lists = list(
                zip(*baseline_Pearson_coeff_maps.values())
            )
        except:
            R2_lists = []
            Pearson_coeff_lists = []
        try:
            data_full[model_name.replace("_{}", "")] = {
                ...
                if len(Pearson_coeff_lists) > 0
                else [],
                "baseline_R2": list(baseline_R2_lists[0])
                if len(baseline_R2_lists) > 0
                else [],
                "baseline_Pearson_coeff": list(baseline_Pearson_coeff_lists[0])
                if len(baseline_Pearson_coeff_lists) > 0
                else [],
            }
        except:
