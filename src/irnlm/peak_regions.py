...
def plot_distrib(
    ...
    label = 'Voxels in Syntactic \npeak regions'  if syntax else 'Voxels in Semantic \npeak regions'
    
    #plt.fill_between(x, y2, step="pre", alpha=0.4)

    ax = sns.distplot(data, hist=False, kde=True, kde_kws={'shade': False, 'linewidth': 3, 'color': 'blue'})
    kde_x, kde_y = ax.lines[0].get_data()

    ax.fill_between(kde_x, kde_y, where=kde_x>threshold, #where=(kde_x<x0) | (kde_x>threshold)
                    interpolate=True, color=color, label=label)
    plt.axvline(threshold, color='black', label=f'90-th percentile: {np.round(threshold, 3)}', linewidth=3)
    if fdr_th is not None:
        plt.axvline(fdr_th, color='green', label=f'Significance: {np.round(fdr_th, 3)}', linewidth=3)
...
def plot_peak_regions(names_restricted, surname, imgs, zmaps, masker, threshold=90, saving_folder='./', ref_img=None, vmax = 1):
    ...
    overlap_masks = [create_mask_from_threshold(img, masker, threshold=threshold, above=True, percentile=True) for img in imgs]
    overlap_masks[0] = math_img('img1*img2', img1=overlap_masks[0], img2=significance_masks[0])
    overlap_masks[1] = math_img('img1*img2', img1=overlap_masks[1], img2=significance_masks[1])

    for i, img in enumerate(imgs):
        plot_name = f'syntax_R_scores_distribution_{surname}_compared_to_BF' if i%2==0 else f'semantic_R_scores_distribution_{surname}_compared_to_BF' 
        #plot_distrib(img, syntax=(i%2==0), plot_name=plot_name, saving_folder=saving_folder, fdr_th=fdr_ths[i])
        plot_distrib(img, masker, syntax=(i%2==0), plot_name=plot_name, saving_folder=saving_folder, fdr_th=None)
    ...
    pairs_overlap_syn_sem_core_indexes = itertools.combinations(np.arange(len(overlap_masks)), 2)
    for pair in pairs_overlap_syn_sem_core_indexes:
        print(f"Overlap between {titles[pair[0]]} and {titles[pair[1]]}: {masks_overlap(overlap_masks[pair[0]], overlap_masks[pair[1]])}")
    ...
    for index, mask in enumerate(overlap_masks):
        print(f"Using {np.sum(overlap_syn_sem_raw.get_fdata())} voxels in mask intersection.")
        print(f"Which means a global intersection of {100*np.sum(overlap_syn_sem_raw.get_fdata())/np.sum(mask.get_fdata())}")
    ...
    for h, hemi in enumerate(hemispheres):
        for v, view in enumerate(views):
            figure, axes = create_grid(nb_rows=1, nb_columns=1, row_size_factor=6, overlapping=6, column_size_factor=12)

            #if hemi=='left' and view=='lateral':
            ax = axes[0][0]
            kwargs = set_projection_params(hemi, view, cmap=new_cmap, 
            inflated=inflated, threshold=1e-15, colorbar=False, symmetric_cbar=False, template=None, figure=figure, ax=ax, vmax=vmax)

            surf_imgs = [surf_projs_syn_sem_core_superimposed[name][f'{hemi}-{view}'] for name in names_restricted]
            d = [np.hstack([
            ...
            for k, s in enumerate(d):
                d[k][np.isnan(s)] = 0
                d[k][s>0] = 1            
            print(f"hemi: {hemi} --- Jaccard score: {jaccard_score(d[0]>0, d[1]>0, average='binary')}")
            ...
            for i in np.unique(surf_img):
                print('-->', hemi, '-', view, '-', i, '-', 100*np.sum(surf_img==i)/10242)
