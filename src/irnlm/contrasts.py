...
def group_analysis(
    ...
    plt.hist(
        np.mean(
            np.stack(masker.transform(imgs), axis=0), 
            axis=0
        ).reshape(-1), 
        bins=200
    )
    plt.show()
    model = SecondLevelModel(smoothing_fwhm=smoothing_fwhm, n_jobs=-1, mask_img=mask_img)
    design_matrix = design_matrix if (design_matrix is not None) else pd.DataFrame([1] * len(imgs),
                                                             columns=['intercept'])
    ...
    mask_img = mask_img if type(mask_img)==nib.nifti1.Nifti1Image else (None if mask_img is None else mask_img.mask_img_)
    thresholded_zmap, fdr_th = threshold_stats_img(stat_img=z_map, alpha=fdr, height_control=height_control, cluster_threshold=cluster_threshold, mask_img=mask_img)
...
def compute_diff_imgs(img1s, img2s):
    ...
    """
    diff_imgs = [math_img('img1-img2', img1=img1, img2=img2) for (img1, img2) in zip(img1s, img2s)]
    return diff_imgs
...
def compute_diff_group_level_maps(name1, name2, name3=None, masker=None, verbose=0):
    ...
    if name3 is not None:
        imgs = {'-vs-'.join(list(data.keys())): [math_img('img1-img2+img3', img1=img1, img2=img2, img3=img3) for (img1, img2, img3) in zip(*imgs)]}
    else:
        imgs = {'-vs-'.join(list(data.keys())): [math_img('img1-img2', img1=img1, img2=img2) for (img1, img2) in zip(*imgs)]}
    zmaps_dict = {}
    effmaps_dict = {}
    masks_dict = {}
    for key in tqdm(imgs.keys()):
    ...
    for i, name in enumerate(surnames):
        ...
        for h, hemi in enumerate(hemispheres):
            for v, view in enumerate(views):
                ...
                inflated=inflated, threshold=threshold, colorbar=plot_cbar, symmetric_cbar=False, template=None, figure=figure, ax=ax, vmax=vmax)
